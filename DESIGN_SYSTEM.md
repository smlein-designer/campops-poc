...
**The variant (visual style) and the `as` prop (HTML tag) are independent.** A Figma "Heading/H4" is a rung in the type scale, not an instruction to render `<h4>` — Figma's own dev-mode output never emits semantic heading tags at all (every text node exports as `<p>`), so document outline is a decision made in code, driven by each page's actual structure. Always pass `as` explicitly rather than trusting the default when a screen's outline requires it — e.g. a page's one `<h1>` might be styled `heading-h4` if that's the visual weight the page calls for; a set of sibling sections should share one heading level even if they're styled at different rungs (see `PublishReadiness` vs `Campsite details` on the editor screens: both `<h2>`, but the former is `heading-h4`/Fraunces via a verified DS component and the latter is a plain Public Sans style — see next paragraph).

**Known DS inconsistency:** the Style Guide's `Heading/H4` rung is defined as Fraunces (display font), but the *actual* screen headings that share its exact size/weight/line-height ("Campsite details," "Amenities," "Availability & Restrictions" on the editor screens) are rendered in Public Sans (body font) in the real screen nodes, not the Style Guide's abstract example. Code follows the real screens (visually confirmed against the actual mockups), not the Style Guide token — so these three headings are NOT `Typography variant="heading-h4"`, they're a plain `text-lg leading-[1.35] font-semibold` with no matching named rung. Flag this back to design rather than "fixing" it by picking one; it's a real disagreement inside the Figma file itself, not a code bug.
